...
import (
	...
	"io"
	"log"
	"nebula/third_party/http/httptrace"
	"nebula/third_party/http/internal/ascii"
	"net"
	"net/textproto"
	"net/url"
	"os"
...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				...
				req.closeBody()
				return nil, fmt.Errorf("nebula/third_party/http: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					req.closeBody()
					return nil, fmt.Errorf("nebula/third_party/http: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		req.closeBody()
		return nil, fmt.Errorf("nebula/third_party/http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var errCannotRewind = errors.New("nebula/third_party/http: cannot rewind body after connection loss")
...
var ErrSkipAltProtocol = errors.New("nebula/third_party/http: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("nebula/third_party/http: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("nebula/third_party/http: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("nebula/third_party/http: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) customDialTLS(ctx context.Context, network, addr string) (conn net.Conn, err error) {
	...
	if conn == nil && err == nil {
		err = errors.New("nebula/third_party/http: Transport.DialTLS or DialTLSContext returned (nil, nil)")
	}
	return
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("nebula/third_party/http: internal error: connCount underflow")
	}
	...
	if q := t.connsPerHostWait[key]; q.len() > 0 {
		...
		if q.len() == 0 {
			...
			// q is a value (like a slice), so we have to stores
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("nebula/third_party/http: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("nebula/third_party/http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("nebula/third_party/http: too many 1xx informational responses")
			}
...
var errTimeout error = &httpError{err: "nebula/third_party/http: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("nebula/third_party/http: request canceled while waiting for connection") // TODO: unify?
...
func (tlsHandshakeTimeoutError) Error() string {
	return "nebula/third_party/http: TLS handshake timeout"
}
