...
import (
	"bytes"
	"io/fs"
	"nebula/third_party/http/internal/testenv"
	"net/url"
	"os"
	"os/exec"
	"reflect"
...
func TestOmitHTTP2(t *testing.T) {
	...
	out, err := exec.Command(goTool, "test", "-short", "-tags=nethttpomithttp2", "nebula/third_party/http").CombinedOutput()
	if err != nil {
...
func TestOmitHTTP2Vet(t *testing.T) {
	t.Parallel()
	goTool := testenv.GoToolPath(t)
	out, err := exec.Command(goTool, "vet", "-tags=nethttpomithttp2", "nebula/third_party/http").CombinedOutput()
	if err != nil {
...
func TestNoUnicodeStrings(t *testing.T) {
	...
	if err := fs.WalkDir(os.DirFS("."), ".", func(path string, d fs.DirEntry, err error) error {
		...
		if path == "internal/ascii" || path == "internal/profile" || path == "internal/testenv" {
			return fs.SkipDir
		}
		if !strings.HasSuffix(path, ".go") ||
			strings.HasSuffix(path, "_test.go") ||
			path == "h2_bundle.go" || path == "header.go" || d.IsDir() {
			return nil
		}
