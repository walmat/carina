...
type http2Framer struct {
	...
	// PUSH_PROMISE + CONTINUATIONs -> http2MetaPushPromiseFrame
...
func http2parseSettingsFrame(_ *http2frameCache, fh http2FrameHeader, p []byte) (http2Frame, error) {
	...
	if v, ok := f.Value(H2SettingInitialWindowSize); ok && v > (1<<31)-1 {
...
func (f *http2SettingsFrame) Value(id H2SettingID) (v uint32, ok bool) {
	f.checkValid()
...
func (f *http2SettingsFrame) Setting(i int) H2Setting {
	buf := f.p
	return H2Setting{
		ID:  H2SettingID(binary.BigEndian.Uint16(buf[i*6 : i*6+2])),
...
func (f *http2SettingsFrame) ForeachSetting(fn func(H2Setting) error) error {
	f.checkValid()
	for i := 0; i < f.NumSettings(); i++ {
...
func (f *http2Framer) WriteSettings(settings ...H2Setting) error {
...
func (p H2PriorityParam) IsZero() bool {
	return p == H2PriorityParam{}
}
...
func http2parsePriorityFrame(_ *http2frameCache, fh http2FrameHeader, payload []byte) (http2Frame, error) {
	...
	return &http2PriorityFrame{
		http2FrameHeader: fh,
		H2PriorityParam: H2PriorityParam{
			Weight:    payload[4],
			StreamDep: streamID,
...
// It will perform exactly one Write to the underlying Writer.
// It is the caller's responsibility to not call other Write methods concurrently.
func (f *http2Framer) WritePriority(streamID uint32, p H2PriorityParam) error {
	if !http2validStreamID(streamID) && !f.AllowIllegalWrites {
		return http2errStreamID
	}
...
// A http2MetaPushPromiseFrame is the representation of one PUSH_PROMISE frame and
// zero or more contiguous CONTINUATION frames and the decoding of
// their HPACK-encoded contents.
//
// This type of frame does not appear on the wire and is only returned
...
// merges them into the provided hf and returns a MetaHeadersFrame
// with the decoded hpack values.
func (fr *http2Framer) readMetaFrame(hf http2continuable) (http2Frame, error) {
	if fr.AllowIllegalReads {
	...
	case *http2HeadersFrame:
		f = &http2MetaHeadersFrame{http2HeadersFrame: orig, Fields: mh.Fields, Truncated: mh.Truncated}
	case *http2PushPromiseFrame:
		f = &http2MetaPushPromiseFrame{http2PushPromiseFrame: orig, Fields: mh.Fields, Truncated: mh.Truncated}
	default:
...
func http2summarizeFrame(f http2Frame) string {
	...
	case *http2SettingsFrame:
		n := 0
		f.ForeachSetting(func(s H2Setting) error {
			n++
			if n == 1 {
...
var http2settingName = map[H2SettingID]string{
...
func (s H2SettingID) String() string {
...
func (sc *http2serverConn) serve() {
	...
	sc.writeFrame(http2FrameWriteRequest{
		write: http2writeSettings{
			{H2SettingMaxFrameSize, sc.srv.maxReadFrameSize()},
			{H2SettingMaxConcurrentStreams, sc.advMaxStreams},
			{H2SettingMaxHeaderListSize, sc.maxHeaderListSize()},
			{H2SettingInitialWindowSize, uint32(sc.srv.initialStreamRecvWindowSize())},
		},
	})
	sc.unackedSettings++
...
func (sc *http2serverConn) processSetting(s H2Setting) error {
	sc.serveG.check()
...
func http2checkPriority(streamID uint32, p H2PriorityParam) error {
...
func (sc *http2serverConn) processPriority(f *http2PriorityFrame) error {
	...
	if err := http2checkPriority(f.StreamID, f.H2PriorityParam); err != nil {
		return err
	}
	sc.writeSched.AdjustStream(f.StreamID, f.H2PriorityParam)
	return nil
}
...
// http2authorityHostPort accepts a given authority (a host/IP, or host:port / ip:port)
...
func (t *http2Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*Http2ClientConn, error) {
	var windowUpdate uint32 = http2transportDefaultStreamFlow
	if t.fpConfig.WindowUpdate > 0 {
		windowUpdate = t.fpConfig.WindowUpdate
	}

	var maxFrameSize uint32 = 16 << 10
	var initialWindowSize uint32 = http2initialWindowSize
	var headerTableSize uint32 = http2initialHeaderTableSize
	var maxConcurrentStreams uint32 = 1000

	var initialSettings []H2Setting
	if t.fpConfig.H2Settings != nil {
		initialSettings = t.fpConfig.H2Settings
		for _, setting := range initialSettings {
			...
			case H2SettingMaxConcurrentStreams:
				maxConcurrentStreams = setting.Val
			case H2SettingMaxFrameSize:
				maxFrameSize = setting.Val
			}
		}
	} else {
		initialSettings = []H2Setting{
			{ID: H2SettingEnablePush, Val: 0},
			{ID: H2SettingInitialWindowSize, Val: http2transportDefaultStreamFlow},
		}
		if max := t.maxHeaderListSize(); max != 0 {
			initialSettings = append(initialSettings, H2Setting{ID: H2SettingMaxHeaderListSize, Val: max})
		}
	}
	...
	cc := &Http2ClientConn{
		...
		nextStreamID:          1,
		maxFrameSize:          maxFrameSize,         // spec default
	...
	if t.fpConfig.InitialStreamID != 0 {
		cc.nextStreamID = 15
	} else if t.AllowHTTP {
		cc.nextStreamID = 3
	}
	...
	cc.fr.WriteWindowUpdate(0, windowUpdate)
	cc.inflow.add(int32(windowUpdate + initialWindowSize))

	for _, pf := range t.fpConfig.PriorityFrames {
		cc.fr.WritePriority(pf.StreamID, pf.Param)
	}
	...
	for len(hdrs) > 0 && cc.werr == nil {
		...
		if first {
			cc.fr.WriteHeaders(http2HeadersFrameParam{
				...
				Priority:      cc.t.fpConfig.HeaderPriority,
			})
			first = false
		} else {
...
func (cc *Http2ClientConn) encodeHeaders(req *Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		// [RFC3986]).
		switch req.FingerprintConfig.H2Headers {
		case H2HeadersAmps, "":
			f(":authority", host)
			m := req.Method
			if m == "" {
		...
		if headerOrder, ok := req.Header[HeaderOrderKey]; ok {
			...
			kvs, sorter = req.Header.sortedKeyValuesBy(order, map[string]bool{HeaderOrderKey: true})
		} else {
			kvs, sorter = req.Header.sortedKeyValues(map[string]bool{HeaderOrderKey: true})
		}
		for _, kv := range kvs {
			k := kv.key
			vv := kv.values
		...
		hasHeaderOrder := reqHasHeaderOrder(req)
		if !hasHeaderOrder && http2shouldSendReqContentLength(req.Method, contentLength) {
			f("content-length", strconv.FormatInt(contentLength, 10))
		}
		if !hasHeaderOrder && addGzipHeader {
			f("accept-encoding", "gzip")
		}
		if !hasHeaderOrder && !didUA {
...
func (cc *Http2ClientConn) newStreamWithId(streamId uint32, incNext bool) *http2clientStream {
	...
	cs.inflow.add(int32(cc.windowUpdate))
...
func (b http2transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("nebula/third_party/http: server replied with more than declared Content-Length; truncated")
...
func (rl *http2clientConnReadLoop) processSettings(f *http2SettingsFrame) error {
	...
	err := f.ForeachSetting(func(s H2Setting) error {
		switch s.ID {
		case H2SettingMaxFrameSize:
...
type http2writeSettings []H2Setting
...
func (s http2writeSettings) writeFrame(ctx http2writeContext) error {
	return ctx.Framer().WriteSettings([]H2Setting(s)...)
}
...
type http2WriteScheduler interface {
	...
	AdjustStream(streamID uint32, priority H2PriorityParam)
...
func (ws *http2priorityWriteScheduler) AdjustStream(streamID uint32, priority H2PriorityParam) {
	if streamID == 0 {
...
func (ws *http2randomWriteScheduler) AdjustStream(streamID uint32, priority H2PriorityParam) {
...
// Http2PushHandler consumes a pushed response.
...
// Http2PushedRequest describes a request that was pushed from the server.
...
// response headers are fully received, ReadResponse will fail and http2PushedRequest
// will be cancelled.
...
func pushedRequestToHTTPRequest(mppf *http2MetaPushPromiseFrame) (*Request, error) {
...
// If Http2PushHandler.HandlePush returns before the pushed stream has completed, the pushed
// stream is canceled.
func handlePushEarlyReturnCancel(pushHandler Http2PushHandler, pushedRequest *Http2PushedRequest) {
