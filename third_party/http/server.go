...
func relevantCaller() runtime.Frame {
	...
	for {
		...
		if !strings.HasPrefix(frame.Function, "nebula/third_party/http.") {
			return frame
		}
		if !more {
			break
...
func (w *response) write(lenData int, dataB []byte, dataS string) (n int, err error) {
	...
	if w.canWriteContinue.isSet() {
		...
		// Tell it not to. The stores must be done while holding the lock
...
var ErrAbortHandler = errors.New("nebula/third_party/http: abort Handler")
...
func (w *response) Hijack() (rwc net.Conn, buf *bufio.ReadWriter, err error) {
	if w.handlerDone.isSet() {
		panic("nebula/third_party/http: Hijack called after ServeHTTP finished")
	}
	if w.wroteHeader {
		w.cw.flush()
	}
...
func (w *response) CloseNotify() <-chan bool {
	if w.handlerDone.isSet() {
		panic("nebula/third_party/http: CloseNotify called after ServeHTTP finished")
	}
...
// listeners.
//
// We stores a pointer to interface in the map set, in case the
