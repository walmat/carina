...
import (
	...
	"nebula/third_party/http/httptest"
	"nebula/third_party/http/httputil"
	"net"
	"net/url"
	"os"
	"reflect"
...
func testWriteHeaderAfterWrite(t *testing.T, h2, hijack bool) {
	...
	wantLog := "http: superfluous response.WriteHeader call from nebula/third_party/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	if hijack {
		wantLog = "http: response.WriteHeader on hijacked connection from nebula/third_party/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	}
