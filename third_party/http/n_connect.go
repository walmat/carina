...
type connectDialer struct {
	...
	cachedH2ClientConn *Http2ClientConn
...
func NewConnectDialer(proxyUrl *url.URL) (proxy.ContextDialer, error) {
...
// DialContext ctx.Value will be inspected for optional ContextKeyHeader{} key, with `Header` value,
...
func (c *connectDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if ctxHeader, ctxHasHeader := ctx.Value(ContextKeyHeader{}).(Header); ctxHasHeader {
		for k, v := range ctxHeader {
			req.Header[k] = v
		}
	}

	connectHttp2 := func(rawConn net.Conn, h2clientConn *Http2ClientConn) (net.Conn, error) {
		req.Proto = "HTTP/2.0"
		req.ProtoMajor = 2
		req.ProtoMinor = 0
		pr, pw := io.Pipe()
	...
	connectHttp1 := func(rawConn net.Conn) (net.Conn, error) {
		...
		resp, err := ReadResponse(bufio.NewReader(rawConn), req)
		if err != nil {
			_ = rawConn.Close()
			return nil, err
		}
