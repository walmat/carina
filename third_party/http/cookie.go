...
func (c *Cookie) String() string {
	...
	if len(c.Domain) > 0 {
		if validCookieDomain(c.Domain) {
			...
			log.Printf("nebula/third_party/http: invalid Cookie.Domain %q; dropping domain attribute", c.Domain)
		}
	}
...
func sanitizeOrWarn(fieldName string, valid func(byte) bool, v string) string {
	...
	for i := 0; i < len(v); i++ {
		...
		log.Printf("nebula/third_party/http: invalid byte %q in %s; dropping invalid bytes", v[i], fieldName)
		ok = false
		break
	}
	if ok {
