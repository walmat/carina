...
import { addWebhook, Field } from '../../../../stores/Main/reducers/webhooks';
...
const AddWebhook = ({ open, setOpen }: CreateWebhookProps) => {
  ...
  const onSubmit = (values: AddWebhookForm) => {
    ...
    if (isValid) {
      setTouched({});
      handleAddWebhook(values).then(() => {});
    }
  };

  const handleAddWebhook = async (values: AddWebhookForm) => {
    try {
      const { url } = values;
      ...
      dispatch(addWebhook({ ...values, type, active: true }));
